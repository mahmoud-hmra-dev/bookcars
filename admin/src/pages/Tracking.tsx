...
const buildVehicleSvgUrl = (bodyColor: string, accentColor: string, rotation: number) => {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="72" height="72" viewBox="0 0 72 72">
      <defs><filter id="shadow" x="-30%" y="-30%" width="160%" height="160%"><feDropShadow dx="0" dy="4" stdDeviation="3" flood-color="rgba(15,23,42,0.35)"/></filter></defs>
      <g transform="rotate(${rotation} 36 36)" filter="url(#shadow)">
        <path d="M36 7C24 7 17 15 17 25v21c0 6 5 11 11 11h16c6 0 11-5 11-11V25C55 15 48 7 36 7Z" fill="${bodyColor}" stroke="#ffffff" stroke-width="3" stroke-linejoin="round"/>
        <path d="M27 17h18c4 0 7 3 7 7v8H20v-8c0-4 3-7 7-7Z" fill="${accentColor}" fill-opacity="0.95"/>
        <path d="M24 37h24v8c0 3-3 6-6 6H30c-3 0-6-3-6-6v-8Z" fill="#ffffff" fill-opacity="0.18"/>
...
const getMovementStatusLabel = (status?: bookcarsTypes.TraccarFleetStatus) => {
  switch (status) {
    case 'moving': return strings.STATUS_MOVING
    case 'idle': return strings.STATUS_IDLE
    case 'stopped': return strings.STATUS_STOPPED
    case 'offline': return strings.STATUS_OFFLINE
    case 'stale': return strings.STATUS_STALE
    case 'noGps': return strings.STATUS_NO_GPS
    case 'unlinked': return strings.STATUS_UNLINKED
    default: return strings.NO_DATA
  }
}

const getEventTypeLabel = (type?: string) => {
  switch (type) {
    case 'geofenceEnter': return strings.EVENT_GEOFENCE_ENTER
    case 'geofenceExit': return strings.EVENT_GEOFENCE_EXIT
    case 'deviceOnline': return strings.EVENT_DEVICE_ONLINE
    case 'deviceOffline': return strings.EVENT_DEVICE_OFFLINE
    case 'ignitionOn': return strings.EVENT_IGNITION_ON
    case 'ignitionOff': return strings.EVENT_IGNITION_OFF
    case 'overspeed': return strings.EVENT_OVERSPEED
    case 'alarm': return strings.EVENT_ALARM
    case 'motion': return strings.EVENT_MOTION
    case 'deviceMoving': return strings.EVENT_DEVICE_MOVING
    case 'deviceStopped': return strings.EVENT_DEVICE_STOPPED
    case 'all': return strings.ALL_EVENTS
    default: return type || strings.NO_DATA
  }
}

const getFleetStatusMeta = (item: FleetCarView, selectedCarId?: string) => {
  if (item.car._id === selectedCarId) {
    return { key: 'selected', label: strings.SELECTED_VEHICLE, markerColor: '#e8f0fe', accentColor: '#dbeafe', clusterColor: '#1d4ed8', chipTone: 'default' as const }
  }
  if (!item.isLinked) {
    return { key: 'unlinked', label: strings.TRACKING_NOT_LINKED, markerColor: '#334155', accentColor: '#1e293b', clusterColor: '#475569', chipTone: 'default' as const }
  }
  if (item.movementStatus === 'moving') {
    return { key: 'moving', label: getMovementStatusLabel(item.movementStatus), markerColor: '#10b981', accentColor: '#064e3b', clusterColor: '#059669', chipTone: 'success' as const }
  }
  if (item.movementStatus === 'idle') {
    return { key: 'idle', label: getMovementStatusLabel(item.movementStatus), markerColor: '#f59e0b', accentColor: '#451a03', clusterColor: '#d97706', chipTone: 'warning' as const }
  }
  if (item.movementStatus === 'stopped') {
    return { key: 'stopped', label: getMovementStatusLabel(item.movementStatus), markerColor: '#3b82f6', accentColor: '#1e3a8a', clusterColor: '#1d4ed8', chipTone: 'default' as const }
  }
  if (item.movementStatus === 'stale') {
    return { key: 'stale', label: getMovementStatusLabel(item.movementStatus), markerColor: '#fb923c', accentColor: '#431407', clusterColor: '#ea580c', chipTone: 'warning' as const }
  }
  if (item.movementStatus === 'noGps') {
    return { key: 'nogps', label: getMovementStatusLabel(item.movementStatus), markerColor: '#a78bfa', accentColor: '#2e1065', clusterColor: '#7c3aed', chipTone: 'default' as const }
  }
  if (item.movementStatus === 'offline') {
    return { key: 'offline', label: getMovementStatusLabel(item.movementStatus), markerColor: '#64748b', accentColor: '#1e293b', clusterColor: '#475569', chipTone: 'default' as const }
  }
  return { key: 'default', label: getMovementStatusLabel(item.movementStatus), markerColor: '#475569', accentColor: '#1e293b', clusterColor: '#334155', chipTone: 'default' as const }
}
...
const buildFleetPinSvgUrl = (fillColor: string, accentColor: string, selected = false) => {
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="74" height="86" viewBox="0 0 74 86">
      <defs><filter id="shadow" x="-20%" y="-20%" width="160%" height="160%"><feDropShadow dx="0" dy="8" stdDeviation="6" flood-color="rgba(0,0,0,0.5)"/></filter></defs>
      <g filter="url(#shadow)">
        <path d="M37 6C22.09 6 10 18.09 10 33c0 22.71 24.08 43.8 25.11 44.69a3 3 0 0 0 3.78 0C39.92 76.8 64 55.71 64 33 64 18.09 51.91 6 37 6Z" fill="${fillColor}" stroke="${strokeColor}" stroke-width="2.5"/>
        <circle cx="37" cy="33" r="16" fill="${accentColor}" fill-opacity="0.85"/>
        <path d="M28 31.5c0-2.49 2.01-4.5 4.5-4.5h9c2.49 0 4.5 2.01 4.5 4.5v5.5H28v-5.5Zm2.5 7h13l1.5 4.5h-2.5a2.25 2.25 0 0 1-4.5 0h-4a2.25 2.25 0 0 1-4.5 0H27l1.5-4.5Z" fill="#ffffff"/>
      </g>
    </svg>
  `.trim()
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`
...
const buildClusterSvgUrl = (size: number, fillColor: string) => {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
      <defs><filter id="shadow" x="-20%" y="-20%" width="160%" height="160%"><feDropShadow dx="0" dy="6" stdDeviation="5" flood-color="rgba(0,0,0,0.4)"/></filter></defs>
      <g filter="url(#shadow)">
        <circle cx="${size / 2}" cy="${size / 2}" r="${(size / 2) - 4}" fill="${fillColor}"/>
        <circle cx="${size / 2}" cy="${size / 2}" r="${(size / 2) - 12}" fill="rgba(255,255,255,0.12)" stroke="rgba(255,255,255,0.2)" stroke-width="1.5"/>
      </g>
    </svg>
  `.trim()
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`
...
const parseEditableGeofence = (geofence: bookcarsTypes.TraccarGeofence): DraftGeofenceShape | null => {
  ...
  if (circleMatch) {
    ...
    const lng = Number.parseFloat(circleMatch[2])
    const radius = Number.parseFloat(circleMatch[3])
    return [lat, lng, radius].every((value) => Number.isFinite(value)) ? { type: 'circle', center: [lat, lng], radius } : null
  }
  try {
    const geometry = wellknown.parse(area)
    if (geometry?.type === 'LineString') {
  ...
  const values = (area.match(/[+-]?\d+(?:\.\d+)?/g) || []).map((value) => Number.parseFloat(value)).filter((value) => Number.isFinite(value))
  const points: LatLngTuple[] = []
  for (let index = 0; index + 1 < values.length; index += 2) {
    points.push(normalizeLatLngOrder(values[index], values[index + 1]))
  }
...
return null
    ...
    const lng = Number.parseFloat(match[2])
    const radius = Number.parseFloat(match[3])
    return [lat, lng, radius].every((value) => Number.isFinite(value)) ? { id, name, shape: 'circle', center: [lat, lng], radius } : null
  }
  if (shapeType === 'RECTANGLE') {
    const values = (geofence.area.match(/[+-]?\d+(?:\.\d+)?/g) || []).map((value) => Number.parseFloat(value)).filter((value) => Number.isFinite(value))
    return values.length >= 4 ? { id, name, shape: 'rectangle', bounds: [[values[0], values[1]], [values[2], values[3]]] } : null
  }
  try {
    const geometry = wellknown.parse(geofence.area || '')
    if (geometry?.type === 'Polygon' || geometry?.type === 'MultiPolygon') {
  ...
  if (values.length >= 6) {
    ...
    if (shapeType === 'LINESTRING') {
      return points.length >= 2 ? { id, name, shape: 'polyline', points, lineWidth: typeof geofence.attributes?.polylineDistance === 'number' ? geofence.attributes.polylineDistance : undefined } : null
    }
    return points.length >= 3 ? { id, name, shape: 'polygon', points } : null
  }
  return null
...
// ─── GOOGLE MAP COMPONENT (unchanged logic) ────────────────────────────────
const GoogleTrackingMap = ({
  mapMode, mapType, fleetCars, selectedFleetCar, currentPoint, currentPosition,
  routePathPoints, routeStartPoint, routeEndPoint, playbackPoint, playbackPosition,
  playbackHeading, heatmapPoints, showHeatmap, stopMarkers, showStops, geofenceShapes,
  draftGeofence, drawingMode, fitRequestToken, onMarkerClick, onDraftGeofenceChange, onDraftGeofenceDrawn,
}: {
  mapMode: FleetMode
  mapType: TrackingMapType
  fleetCars: FleetCarView[]
...
return
    ...
    let hasBounds = false
    const extend = (point: LatLngTuple) => {
 bounds.extend(toGoogleLatLng(point)); hasBounds = true 
}
    if (mapMode === 'fleet') {
      fleetMarkers.forEach((item) => item.currentPoint && extend(item.currentPoint))
    } else {
...
extend(shape.center)
        ...
        shape.points?.forEach(extend)
        if (shape.bounds) {
 extend(shape.bounds[0]); extend(shape.bounds[1]) 
}
        if (shape.geojson) {
extractGeoJsonPaths(shape.geojson).forEach((ring) => ring.forEach(extend))
}
...
map.fitBounds(bounds, 40)
 ...
 map.setCenter(toGoogleLatLng(currentPoint || DEFAULT_CENTER)); map.setZoom(currentPoint ? 12 : 7) 
}
  }, [currentPoint, fleetMarkers, fitRequestToken, geofenceShapes, googleMaps, mapMode, routePathPoints])
...
return <div className="tracking-map-empty"><Typography variant="body2">{commonStrings.LOADING}</Typography></div>
  ...
  const buildMarkerIcon = (color: string, scale: number): google.maps.Symbol => ({
    path: googleMaps.SymbolPath.CIRCLE, fillColor: color, fillOpacity: 1, strokeColor: '#ffffff', strokeWeight: 2, scale,
  })

  const infoPoint = mapMode === 'fleet' ? selectedFleetCar?.currentPoint || null : playbackPoint || currentPoint
  const infoPosition = mapMode === 'fleet' ? selectedFleetCar?.position || null : playbackPosition || currentPosition
  const resolvedPlaybackHeading = typeof playbackPosition?.course === 'number' && Number.isFinite(playbackPosition.course) ? playbackPosition.course : playbackHeading || 0
  const resolvedCurrentHeading = typeof currentPosition?.course === 'number' && Number.isFinite(currentPosition.course) ? currentPosition.course : 0

  const playbackVehicleIcon: google.maps.Icon = {
    url: buildVehicleSvgUrl('#f97316', '#fde68a', resolvedPlaybackHeading),
    scaledSize: new googleMaps.Size(44, 44), anchor: new googleMaps.Point(22, 22),
  }
  const currentVehicleIcon: google.maps.Icon = {
    url: buildVehicleSvgUrl('#60a5fa', '#1e3a8a', resolvedCurrentHeading),
    scaledSize: new googleMaps.Size(42, 42), anchor: new googleMaps.Point(21, 21),
  }
  const stopMarkerIcon: google.maps.Symbol = {
    path: googleMaps.SymbolPath.CIRCLE, fillColor: '#f59e0b', fillOpacity: 1, strokeColor: '#ffffff', strokeWeight: 2, scale: 7,
  }
  const activeDrawingMode = drawingMode ? (
    drawingMode === 'circle' ? googleMaps.drawing.OverlayType.CIRCLE
      : drawingMode === 'polygon' ? googleMaps.drawing.OverlayType.POLYGON
        : googleMaps.drawing.OverlayType.POLYLINE
  ) : null

  const fleetClusterStyles = [
    { url: buildClusterSvgUrl(52, '#1d4ed8'), height: 52, width: 52, textColor: '#ffffff', textSize: 14 },
    { url: buildClusterSvgUrl(60, '#0f766e'), height: 60, width: 60, textColor: '#ffffff', textSize: 15 },
    { url: buildClusterSvgUrl(70, '#7c3aed'), height: 70, width: 70, textColor: '#ffffff', textSize: 17 },
  ]
...
return
  ...
  const drawingOptions = {
    circleOptions: { clickable: false, editable: false, fillColor: '#fb923c', fillOpacity: 0.16, strokeColor: '#f97316', strokeWeight: 3 },
    polygonOptions: { clickable: false, editable: false, fillColor: '#fb923c', fillOpacity: 0.16, strokeColor: '#f97316', strokeWeight: 3 },
    polylineOptions: { clickable: false, editable: false, strokeColor: '#f97316', strokeWeight: 4, strokeOpacity: 0.95 },
    drawingControl: false,
  } satisfies google.maps.drawing.DrawingManagerOptions

  const heatmapData = !googleMaps.visualization ? [] : heatmapPoints.map((item) => ({
    location: new googleMaps.LatLng(item.point[0], item.point[1]), weight: item.weight,
  }))
  ...
  const darkMapStyle = [
    ...
    { featureType: 'road', elementType: 'geometry', stylers: [{ color: '#1a2d4a' }] },
    ...
    { featureType: 'poi', elementType: 'geometry', stylers: [{ color: '#0d1e2e' }] },
    { featureType: 'poi', elementType: 'labels.text.fill', stylers: [{ color: '#2a4060' }] },
    { featureType: 'poi.park', elementType: 'geometry', stylers: [{ color: '#0b1c2a' }] },
    { featureType: 'transit', elementType: 'geometry', stylers: [{ color: '#0d1e2e' }] },
 ...
 mapRef.current = map 
      ...
      options={{
        ...
        zoomControlOptions: { position: googleMaps.ControlPosition.LEFT_CENTER },
        backgroundColor: '#0a1628',
      }}
    >
      {mapMode === 'single' && showHeatmap && heatmapData.length > 0 && (
        <HeatmapLayer data={heatmapData} options={{ radius: 26, opacity: 0.55, gradient: ['rgba(59,130,246,0)', 'rgba(59,130,246,0.35)', 'rgba(14,165,233,0.55)', 'rgba(250,204,21,0.7)', 'rgba(249,115,22,0.88)', 'rgba(220,38,38,0.96)'] }} />
      )}
      {mapMode === 'single' && activeDrawingMode && (
        <DrawingManager
          drawingMode={activeDrawingMode}
          options={drawingOptions}
          onCircleComplete={(circle) => {
            const center = circle.getCenter(); const radius = circle.getRadius(); circle.setMap(null)
            if (center && Number.isFinite(radius)) {
onDraftGeofenceDrawn({ type: 'circle', center: fromGoogleLatLng(center), radius })
}
          }}
          onPolygonComplete={(polygon) => {
            const points = fromGooglePath(polygon.getPath()); polygon.setMap(null)
            if (points.length >= 3) {
onDraftGeofenceDrawn({ type: 'polygon', points })
}
          }}
          onPolylineComplete={(polyline) => {
            const points = fromGooglePath(polyline.getPath()); polyline.setMap(null)
            if (points.length >= 2) {
onDraftGeofenceDrawn({ type: 'polyline', points })
}
          }}
      ...
      {mapMode === 'single' && geofenceShapes.map((geofence) => {
        if (geofence.shape === 'circle' && geofence.center && geofence.radius) {
return <CircleF key={`${geofence.id}`} center={toGoogleLatLng(geofence.center)} radius={geofence.radius} options={{ strokeColor: '#22d3ee', fillColor: '#0891b2', fillOpacity: 0.12, strokeWeight: 2 }} />
}
        if (geofence.shape === 'rectangle' && geofence.bounds) {
          const north = Math.max(geofence.bounds[0][0], geofence.bounds[1][0])
          const south = Math.min(geofence.bounds[0][0], geofence.bounds[1][0])
          const east = Math.max(geofence.bounds[0][1], geofence.bounds[1][1])
          const west = Math.min(geofence.bounds[0][1], geofence.bounds[1][1])
          return <RectangleF key={`${geofence.id}`} bounds={{ north, south, east, west }} options={{ strokeColor: '#22d3ee', fillColor: '#0891b2', fillOpacity: 0.12, strokeWeight: 2 }} />
        }
        if (geofence.shape === 'polygon' && geofence.points) {
return <PolygonF key={`${geofence.id}`} paths={geofence.points.map(toGoogleLatLng)} options={{ strokeColor: '#22d3ee', fillColor: '#0891b2', fillOpacity: 0.12, strokeWeight: 2 }} />
}
        if (geofence.shape === 'polyline' && geofence.points) {
return <PolylineF key={`${geofence.id}`} path={geofence.points.map(toGoogleLatLng)} options={{ strokeColor: '#22d3ee', strokeWeight: 3, strokeOpacity: 0.9 }} />
}
        if (geofence.shape === 'geojson' && geofence.geojson) {
return extractGeoJsonPaths(geofence.geojson).map((ring, index) => (
            <PolygonF key={`${geofence.id}-${index}`} paths={ring.map(toGoogleLatLng)} options={{ strokeColor: '#22d3ee', fillColor: '#0891b2', fillOpacity: 0.12, strokeWeight: 2 }} />
          ))
}
        return null
      })}
      {mapMode === 'single' && draftGeofence?.type === 'circle' && (
        <CircleF center={toGoogleLatLng(draftGeofence.center)} radius={draftGeofence.radius}
          options={{ strokeColor: '#f97316', fillColor: '#fb923c', fillOpacity: 0.14, strokeWeight: 2.5, zIndex: 90 }}
          editable draggable
          onLoad={(circle) => {
 draftCircleRef.current = circle 
}}
 ...
 draftCircleRef.current = null 
          ...
          onCenterChanged={syncDraftCircle} onRadiusChanged={syncDraftCircle} onDragEnd={syncDraftCircle} onMouseUp={syncDraftCircle}
        />
      )}
      {mapMode === 'single' && draftGeofence?.type === 'polygon' && (
        <PolygonF paths={draftGeofence.points.map(toGoogleLatLng)}
          options={{ strokeColor: '#f97316', fillColor: '#fb923c', fillOpacity: 0.14, strokeWeight: 2.5, zIndex: 90 }}
          editable draggable
          onLoad={(polygon) => {
 draftPolygonRef.current = polygon 
 ...
 draftPolygonRef.current = null 
          ...
          onEdit={syncDraftPolygon} onDragEnd={() => syncDraftPolygon()} onMouseUp={() => syncDraftPolygon()}
        />
      )}
      {mapMode === 'single' && draftGeofence?.type === 'polyline' && (
        <PolylineF path={draftGeofence.points.map(toGoogleLatLng)}
          options={{ strokeColor: '#f97316', strokeWeight: 3.5, strokeOpacity: 0.95, zIndex: 90 }}
          editable draggable
          onLoad={(polyline) => {
 draftPolylineRef.current = polyline; attachDraftPolylineListeners(polyline) 
}}
          onUnmount={() => {
 clearDraftPolylineListeners(); draftPolylineRef.current = null 
}}
          onDragEnd={syncDraftPolyline} onMouseUp={syncDraftPolyline}
        />
      )}
      {mapMode === 'single' && routePathPoints.length > 1 && <PolylineF path={routePathPoints.map(toGoogleLatLng)} options={{ strokeColor: '#3b82f6', strokeWeight: 3.5, strokeOpacity: 0.85 }} />}
      {mapMode === 'single' && routeStartPoint && routePathPoints.length > 1 && <MarkerF position={toGoogleLatLng(routeStartPoint)} title={strings.ROUTE_START} icon={buildMarkerIcon('#10b981', 7)} />}
      {mapMode === 'single' && routeEndPoint && routePathPoints.length > 1 && <MarkerF position={toGoogleLatLng(routeEndPoint)} title={strings.ROUTE_END} icon={buildMarkerIcon('#f59e0b', 7)} />}
      {mapMode === 'single' && showStops && stopMarkers.map((stop, index) => (
        <MarkerF key={stop.id} position={toGoogleLatLng(stop.point)} title={`Stop ${index + 1}: ${formatDuration(stop.durationMs)}`} icon={stopMarkerIcon} label={{ text: `${index + 1}`, color: '#ffffff', fontWeight: '700', fontSize: '10px' }} />
      ))}
      {mapMode === 'single' && playbackPoint && <MarkerF position={toGoogleLatLng(playbackPoint)} title={strings.ROUTE_PLAYBACK} icon={playbackVehicleIcon} />}
      {mapMode === 'single' && currentPoint && !playbackPoint && <MarkerF position={toGoogleLatLng(currentPoint)} title={selectedFleetCar?.car.name || strings.SELECT_CAR} icon={currentVehicleIcon} />}
      {mapMode === 'fleet' && fleetMarkers.length > 0 && (
        <MarkerClustererF averageCenter enableRetinaIcons gridSize={56} minimumClusterSize={2} styles={fleetClusterStyles}>
          {(clusterer) => (
            <>
              {fleetMarkers.map((item) => {
                const statusMeta = getFleetStatusMeta(item, selectedFleetCar?.car._id)
                const isSelected = item.car._id === selectedFleetCar?.car._id
                const fleetMarkerIcon: google.maps.Icon = {
                  url: buildFleetPinSvgUrl(statusMeta.markerColor, statusMeta.accentColor, isSelected),
                  scaledSize: new googleMaps.Size(isSelected ? 38 : 32, isSelected ? 44 : 37),
                  anchor: new googleMaps.Point(isSelected ? 19 : 16, isSelected ? 40 : 33),
                }
                return (
                  <MarkerF key={item.car._id} clusterer={clusterer} position={toGoogleLatLng(item.currentPoint as LatLngTuple)}
                    title={item.car.name} onClick={() => onMarkerClick(item.car._id)} icon={fleetMarkerIcon}
                    zIndex={isSelected ? 120 : item.movementStatus === 'moving' ? 80 : 40}
                  />
                )
              })}
            </>
...
const Tracking = () => {
 ...
 setActivePanelSection(section) 
  ...
  useEffect(() => {
    ...
    const loadVehicleCommandTypes = async () => {
      if (!selectedCar?.tracking?.deviceId || !integrationEnabled) {
 setCommandTypes([]); setSelectedCommandType(''); return 
}
      try {
        const types = await TraccarService.getCommandTypes(selectedCar._id)
        if (!active) {
return
        ...
        setCommandTypes(types)
        const defaultType = types[0]?.type || ''
        setSelectedCommandType((current) => (current && types.some((type) => type.type === current) ? current : defaultType))
      } catch {
 if (active) {
 setCommandTypes([]); setSelectedCommandType('') 
} 
}
    }
    void loadVehicleCommandTypes()
...
setCommandTextChannel(match.textChannel)
 ...
 setGeofences([]); return 
      ...
      try {
 const data = await TraccarService.getGeofences(selectedCar._id); if (active) {
setGeofences(data)
} 
} catch {
 if (active) {
setGeofences([])
 ...
 active = false 
  ...
  const linkedGeofenceIds = useMemo(() => new Set(geofences.map((geofence) => geofence.id).filter((id): id is number => typeof id === 'number')), [geofences])

  const resetGeofenceForm = () => {
    setEditingGeofenceId(null); setGeofenceFormName(''); setGeofenceFormDescription(''); setGeofenceFormType('circle')
    setGeofenceFormRadius('200'); setGeofenceFormPolylineDistance('25'); setGeofenceDraft(null); setGeofenceDrawingMode(null)
  }

  const populateGeofenceForm = (geofence: bookcarsTypes.TraccarGeofence) => {
    const draft = parseEditableGeofence(geofence)
    if (!draft) {
 helper.error(null, strings.UNSUPPORTED_GEOFENCE); return 
}
    focusPanelSection('geofences')
    setEditingGeofenceId(typeof geofence.id === 'number' ? geofence.id : null)
    setGeofenceFormName(geofence.name || ''); setGeofenceFormDescription(geofence.description || '')
    setGeofenceFormPolylineDistance(`${typeof geofence.attributes?.polylineDistance === 'number' ? geofence.attributes.polylineDistance : 25}`)
    setGeofenceFormType(draft.type); setGeofenceFormRadius(draft.type === 'circle' ? `${Math.round(draft.radius * 100) / 100}` : '200')
    setGeofenceDraft(draft); setGeofenceDrawingMode(null); setMapMode('single'); setMapFitRequestToken((prev) => prev + 1)
  }
...
throw new Error(commonStrings.FIELD_NOT_VALID)
      ...
      return { name, description: geofenceFormDescription.trim() || undefined, area: `CIRCLE (${geofenceDraft.center[0]} ${geofenceDraft.center[1]}, ${radius})`, attributes: {} }
    }
    const points = geofenceDraft.points
    if (geofenceDraft.type === 'polyline') {
      if (points.length < 2) {
...
throw new Error(commonStrings.FIELD_NOT_VALID)
      ...
      return { name, description: geofenceFormDescription.trim() || undefined, area: `LINESTRING (${points.map(([lat, lng]) => `${lat} ${lng}`).join(', ')})`, attributes: { polylineDistance } }
    }
    if (points.length < 3) {
throw new Error(commonStrings.FIELD_NOT_VALID)
}
    const closedPoints = closePolygon(points)
    return { name, description: geofenceFormDescription.trim() || undefined, area: `POLYGON ((${closedPoints.map(([lat, lng]) => `${lat} ${lng}`).join(', ')}))`, attributes: {} }
  }

  const handleGeofenceTypeChange = (nextType: GeofenceEditorType) => {
    setGeofenceFormType(nextType); setGeofenceDrawingMode(null)
    if (geofenceDraft && geofenceDraft.type !== nextType) {
setGeofenceDraft(null)
}
    if (nextType === 'circle' && (!geofenceDraft || geofenceDraft.type !== 'circle')) {
setGeofenceFormRadius('200')
  ...
  const handleStartGeofenceDrawing = () => {
    focusPanelSection('geofences'); setMapMode('single')
    setGeofenceDrawingMode((current) => (current === geofenceFormType ? null : geofenceFormType))
    setMapFitRequestToken((prev) => prev + 1)
  }

  const handleClearGeofenceDrawing = () => {
 setGeofenceDraft(null); setGeofenceDrawingMode(null) 
}

  const handleDraftGeofenceChange = (draft: DraftGeofenceShape) => {
    setGeofenceDraft(draft); setGeofenceFormType(draft.type)
    if (draft.type === 'circle') {
setGeofenceFormRadius(`${Math.round(draft.radius * 100) / 100}`)
}
  }
  ...
  const handleDraftGeofenceDrawn = (draft: DraftGeofenceShape) => {
    handleDraftGeofenceChange(draft); setGeofenceDrawingMode(null); setMapFitRequestToken((prev) => prev + 1)
  }
...
setGeofenceDraft({ ...geofenceDraft, radius })
  ...
  const fleetCars = useMemo<FleetCarView[]>(() => {
    const result = cars.map((car) => {
      ...
      const speedKmh = snapshot?.speedKmh || 0
      return {
        car, snapshot, position, currentPoint: toLatLng(position),
        deviceName: snapshot?.deviceName || car.tracking?.deviceName || '',
        deviceStatus, isLinked, isOnline: deviceStatus.trim().toLowerCase() === 'online',
        isLive: !!position, movementStatus, stale, staleMinutes: snapshot?.staleMinutes,
        speedKmh, lastSeen: formatTimestamp(snapshot?.lastPositionAt || getPositionTimestamp(position) || snapshot?.lastDeviceUpdate || snapshot?.lastSyncedAt),
      }
    })
    result.sort((left, right) => {
      const statusPriority = { moving: 0, idle: 1, stopped: 2, stale: 3, noGps: 4, offline: 5, unlinked: 6 } as const
      const leftPriority = statusPriority[left.movementStatus]
      const rightPriority = statusPriority[right.movementStatus]
      if (leftPriority !== rightPriority) {
return leftPriority - rightPriority
...
return left.isLinked ? -1 : 1
  ...
  const supplierOptions = useMemo(() => (
    [...new Set(cars.map((car) => car.supplier?.fullName?.trim()).filter((name): name is string => !!name))].sort((left, right) => left.localeCompare(right))
  ), [cars])
  ...
  const selectedFleetCar = useMemo(() => fleetCars.find((item) => item.car._id === selectedCarId) || null, [fleetCars, selectedCarId])
  const managedGeofences = useMemo(() => [...allGeofences].sort((left, right) => (left.name || '').localeCompare(right.name || '')), [allGeofences])
  const currentPosition = positions[0] || selectedFleetCar?.position || null
  const currentPoint = useMemo(() => toLatLng(currentPosition), [currentPosition])

  const routeFrames = useMemo<RouteFrame[]>(() => route.map((position) => {
    const point = toLatLng(position)
    if (!point) {
return null
}
    return { point, position, timestampMs: getDateMs(getPositionTimestamp(position)), speed: typeof position.speed === 'number' && Number.isFinite(position.speed) ? position.speed : 0 }
  }).filter((frame): frame is RouteFrame => frame !== null), [route])

  const rawRoutePoints = useMemo(() => routeFrames.map((frame) => frame.point), [routeFrames])
  const routePathPoints = useMemo(() => (snappedRoute.displayPoints.length > 1 ? snappedRoute.displayPoints : rawRoutePoints), [rawRoutePoints, snappedRoute.displayPoints])
  const geofenceShapes = useMemo(() => geofences.map(parseGeofenceArea).filter((shape): shape is ParsedGeofence => shape !== null), [geofences])
  const visibleGeofenceShapes = useMemo(() => (editingGeofenceId === null ? geofenceShapes : geofenceShapes.filter((shape) => `${shape.id}` !== `${editingGeofenceId}`)), [editingGeofenceId, geofenceShapes])
  const routeStartPoint = routePathPoints[0] || null
  const routeEndPoint = routePathPoints.length > 1 ? routePathPoints[routePathPoints.length - 1] : routePathPoints[0] || null
  const boundedPlaybackIndex = routeFrames.length > 0 ? Math.min(playbackIndex, routeFrames.length - 1) : 0
  const playbackFrame = routeFrames[boundedPlaybackIndex] || null
  const playbackPoint = snappedRoute.playbackPoints[boundedPlaybackIndex] || playbackFrame?.point || null
  const nextPlaybackPoint = snappedRoute.playbackPoints[boundedPlaybackIndex + 1] || routeFrames[boundedPlaybackIndex + 1]?.point || null
  const playbackHeading = playbackPoint && nextPlaybackPoint ? calculateBearing(playbackPoint, nextPlaybackPoint) : null
  const playbackSpeedKmh = playbackFrame ? playbackFrame.speed * 1.852 : 0
  const playbackProgress = routeFrames.length > 1 ? Math.round((boundedPlaybackIndex / (routeFrames.length - 1)) * 100) : 0
 ...
 cluster = []; return 
      ...
      const centerLat = cluster.reduce((sum, item) => sum + item.point[0], 0) / cluster.length
      const centerLng = cluster.reduce((sum, item) => sum + item.point[1], 0) / cluster.length
      stops.push({ id: `stop-${startedAt}-${endedAt}-${stops.length}`, point: [centerLat, centerLng], startedAt, endedAt, durationMs, pointCount: cluster.length })
      cluster = []
    }
    routeFrames.forEach((frame, index) => {
      const point = snappedRoute.playbackPoints[index] || frame.point
 ...
 flushCluster(); return 
      ...
      if (cluster.length === 0) {
 cluster = [{ frame, point }]; return 
}
      if (haversineDistanceMeters(cluster[0].point, point) <= STOP_RADIUS_METERS) {
cluster.push({ frame, point })
} else {
 flushCluster(); cluster = [{ frame, point }] 
}
    })
    flushCluster()
    return stops
  ...
  const routeStopDurationMs = useMemo(() => (
    vehicleReports.stops.length > 0 ? vehicleReports.stops.reduce((sum, stop) => sum + (stop.duration || 0), 0) : detectedStops.reduce((sum, stop) => sum + stop.durationMs, 0)
  ), [detectedStops, vehicleReports.stops])
  const routeDistanceMeters = vehicleReports.summary?.distance ?? vehicleReports.trips.reduce((sum, trip) => sum + (trip.distance || 0), 0)
  const routeAverageSpeed = vehicleReports.summary?.averageSpeed
  const routeMaxSpeed = vehicleReports.summary?.maxSpeed
  ...
  const onlineCarsCount = fleetHealth?.onlineCars ?? fleetCars.filter((item) => item.isOnline).length
  const canLoadTracking = !!selectedCar?.tracking?.deviceId && integrationEnabled
  const hasMapData = mapMode === 'fleet' ? liveCarsCount > 0 : !!currentPoint || routePathPoints.length > 0 || visibleGeofenceShapes.length > 0 || geofenceDraftReady || geofenceDrawingMode !== null
  const selectedStatusLabel = selectedFleetCar?.isLinked ? getMovementStatusLabel(selectedFleetCar.movementStatus) : strings.TRACKING_NOT_LINKED
  const selectedStatusTone = selectedFleetCar?.movementStatus === 'moving' ? 'success' : selectedFleetCar?.movementStatus === 'idle' || selectedFleetCar?.movementStatus === 'stale' ? 'warning' : 'default'
  const routeListPreview = routeFrames.slice(0, 10)
  const stopListPreview = (vehicleReports.stops.length > 0
    ? vehicleReports.stops.slice(0, 6).map((stop, index) => ({ id: `report-stop-${index}-${stop.startTime || stop.endTime || index}`, point: [stop.latitude || 0, stop.longitude || 0] as LatLngTuple, startedAt: getDateMs(stop.startTime), endedAt: getDateMs(stop.endTime), durationMs: stop.duration || 0, pointCount: 0 }))
    : detectedStops.slice(0, 6))

  useEffect(() => {
    let active = true
    setPlaybackPlaying(false); setPlaybackIndex(0)
    if (rawRoutePoints.length < 2) {
 setSnappedRoute({ displayPoints: rawRoutePoints, playbackPoints: rawRoutePoints, mode: 'idle' }); return () => {
 active = false 
} 
}
    setSnappedRoute({ displayPoints: rawRoutePoints, playbackPoints: rawRoutePoints, mode: 'loading' })
    const snapRoute = async () => {
      try {
        const result = await snapRouteToRoads(rawRoutePoints as RoadPoint[], env.GOOGLE_MAPS_API_KEY)
        if (active) {
setSnappedRoute({ displayPoints: result.displayPoints, playbackPoints: result.playbackPoints, mode: 'snapped' })
}
      } catch {
 if (active) {
setSnappedRoute({ displayPoints: rawRoutePoints, playbackPoints: rawRoutePoints, mode: 'raw' })
...
return
    ...
    if (boundedPlaybackIndex >= routeFrames.length - 1) {
 setPlaybackPlaying(false); return 
}
    const currentFrame = routeFrames[boundedPlaybackIndex]
    const nextFrame = routeFrames[boundedPlaybackIndex + 1]
    const deltaMs = currentFrame.timestampMs > 0 && nextFrame.timestampMs > currentFrame.timestampMs ? nextFrame.timestampMs - currentFrame.timestampMs : 4000
    const delay = Math.max(140, Math.min(1200, deltaMs / playbackSpeed))
    const timer = window.setTimeout(() => {
 setPlaybackIndex((current) => Math.min(current + 1, routeFrames.length - 1)) 
}, delay)
 ...
 window.clearTimeout(timer) 
  ...
  const resetTrackingData = () => {
    setPositions([]); setRoute([]); setVehicleReports({ summary: null, trips: [], stops: [] })
    setSnappedRoute({ displayPoints: [], playbackPoints: [], mode: 'idle' }); setPlaybackPlaying(false); setPlaybackIndex(0); setGeofences([])
  }

  const selectCar = (carId: string) => {
 setSelectedCarId(carId); resetTrackingData() 
}
  const focusSelectedVehicleOnMap = () => {
 if (!selectedCarId) {
return
} setMapMode('single'); setMapFitRequestToken((prev) => prev + 1) 
}
  const focusFleetOnMap = () => {
 setMapMode('fleet'); setMapFitRequestToken((prev) => prev + 1) 
}

  const loadCars = async () => {
    const payload: bookcarsTypes.GetCarsPayload = {
      suppliers: [], carType: bookcarsHelper.getAllCarTypes(),
      gearbox: [bookcarsTypes.GearboxType.Automatic, bookcarsTypes.GearboxType.Manual],
      mileage: [bookcarsTypes.Mileage.Limited, bookcarsTypes.Mileage.Unlimited],
      fuelPolicy: bookcarsHelper.getAllFuelPolicies(), deposit: -1,
      availability: [bookcarsTypes.Availablity.Available, bookcarsTypes.Availablity.Unavailable],
      ranges: bookcarsHelper.getAllRanges(), multimedia: [], rating: -1, seats: -1,
    }
    const suppliers = await SupplierService.getAllSuppliers()
    payload.suppliers = bookcarsHelper.flattenSuppliers(suppliers)
    const firstPage = await CarService.getCars('', payload, 1, CARS_FETCH_SIZE)
...
return firstRows
  ...
  const loadFleetOverview = async () => {
    if (!integrationEnabled) {
 setFleetOverview([]); setFleetHealth(null); return 
}
    const data = await TraccarService.getFleetOverview()
    setFleetOverview(data.items); setFleetHealth(data.health); setLastOperationalRefreshAt(new Date(data.health.lastRefreshAt))
  }
  const loadDevices = async () => {
 if (!integrationEnabled) {
 setDevices([]); return 
  ...
  const loadAllGeofences = async () => {
 if (!integrationEnabled) {
 setAllGeofences([]); return 
}; setAllGeofences(await TraccarService.getAllGeofences()) 
}

  const loadEventCenter = async (scope = eventScope) => {
    if (!integrationEnabled) {
 setEventCenterItems([]); return 
}
    const scopedCarId = scope === 'vehicle' ? selectedCar?._id : undefined
    if (scope === 'vehicle' && !scopedCarId) {
 setEventCenterItems([]); return 
    ...
    setEventCenterItems(await TraccarService.getEventCenter({
      carId: scopedCarId, from: new Date(from).toISOString(), to: new Date(to).toISOString(),
      types: eventTypeFilter === 'all' ? undefined : [eventTypeFilter], limit: EVENT_CENTER_LIMIT,
    }))
  }

  const refreshOperationalData = async ({ includeMetadata = false, includeEventCenter = true, silent = false }: { includeMetadata?: boolean; includeEventCenter?: boolean; silent?: boolean } = {}) => {
    if (!integrationEnabled) {
return
}
    if (!silent) {
...
setLoading(false)
 ...
 await refreshOperationalData({ includeMetadata: true }) 
  ...
  const handleRefreshGeofenceLibrary = async () => {
    focusPanelSection('geofences'); if (!integrationEnabled) {
return
}
    setLoading(true); try {
 await loadAllGeofences() 
...
return
    ...
    if (!deviceId) {
 helper.error(null, commonStrings.FIELD_NOT_VALID); return 
}
    setLoading(true)
    try {
      const tracking = await TraccarService.linkDevice(selectedCar._id, { deviceId: Number.parseInt(deviceId, 10), deviceName, notes, enabled: trackingEnabled })
      const updated = { ...selectedCar, tracking }
      setCars((prev) => prev.map((car) => (car._id === updated._id ? updated : car)))
      await Promise.all([loadFleetOverview(), loadDevices()]); helper.info(commonStrings.UPDATED)
    } catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
...
return
    ...
    try {
      ...
      const updated = { ...selectedCar, tracking }
      setCars((prev) => prev.map((car) => (car._id === updated._id ? updated : car)))
      resetTrackingData(); await loadFleetOverview(); helper.info(commonStrings.UPDATED)
    } catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
  ...
  const handleFetchPositions = async () => {
    focusPanelSection('vehicle'); if (!selectedCar) {
return
}
    setLoading(true)
    try {
 setPositions(await TraccarService.getPositions(selectedCar._id)); setMapMode('single'); setMapFitRequestToken((prev) => prev + 1) 
} catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
  ...
  const handleFetchRoute = async () => {
    focusPanelSection('route'); if (!selectedCar) {
return
}
    setLoading(true)
    try {
      ...
      setRoute(routeData); setVehicleReports(reportsData); setMapMode('single'); setMapFitRequestToken((prev) => prev + 1)
    } catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
  ...
  const handleFetchGeofences = async () => {
    focusPanelSection('geofences'); if (!selectedCar) {
return
}
    setLoading(true)
    try {
 setGeofences(await TraccarService.getGeofences(selectedCar._id)); setMapMode('single'); setMapFitRequestToken((prev) => prev + 1) 
} catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
...
await TraccarService.createGeofence(payload)
      ...
      await Promise.all([loadAllGeofences(), canLoadTracking && selectedCar ? TraccarService.getGeofences(selectedCar._id).then(setGeofences) : Promise.resolve()])
      resetGeofenceForm(); helper.info(commonStrings.UPDATED)
    } catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
...
return
    ...
    setLoading(true); try {
 setGeofences(await TraccarService.linkGeofence(selectedCar._id, geofenceId)); helper.info(commonStrings.UPDATED) 
} catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
...
return
    ...
    setLoading(true); try {
 setGeofences(await TraccarService.unlinkGeofence(selectedCar._id, geofenceId)); helper.info(commonStrings.UPDATED) 
} catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
...
resetGeofenceForm()
 ...
 setLoading(false) 
  ...
  const handleFetchEvents = async () => {
    focusPanelSection('events'); setLoading(true)
    try {
 await loadEventCenter() 
} catch (err) {
 helper.error(err) 
...
return
    ...
    try {
      const [currentData, routeData, geofenceData, reportsData, eventData] = await Promise.all([
        ...
        TraccarService.getGeofences(selectedCar._id),
        TraccarService.getVehicleReports(selectedCar._id, new Date(from).toISOString(), new Date(to).toISOString()),
        TraccarService.getEventCenter({ carId: selectedCar._id, from: new Date(from).toISOString(), to: new Date(to).toISOString(), types: eventTypeFilter === 'all' ? undefined : [eventTypeFilter], limit: EVENT_CENTER_LIMIT }),
      ])
      setPositions(currentData); setRoute(routeData); setGeofences(geofenceData); setVehicleReports(reportsData); setEventCenterItems(eventData)
      setMapMode('single'); setMapFitRequestToken((prev) => prev + 1)
    } catch (err) {
 helper.error(err) 
} finally {
...
setPlaybackIndex(0)
    ...
    setPlaybackPlaying((current) => !current); setMapMode('single'); setMapFitRequestToken((prev) => prev + 1)
  }
 ...
 setMapMode('single'); setMapFitRequestToken((prev) => prev + 1) 
  ...
  const handlePlaybackScrub = (_event: Event, value: number | number[]) => {
    const nextIndex = Array.isArray(value) ? value[0] : value
    setPlaybackPlaying(false); setPlaybackIndex(nextIndex)
  }
...
return
 ...
 helper.error(null, commonStrings.FIELD_NOT_VALID); return 
    ...
    setLoading(true)
    try {
 await TraccarService.sendCommand(selectedCar._id, { type: selectedCommandType, textChannel: commandTextChannel, attributes: parsedAttributes }); helper.info(commonStrings.UPDATED) 
} catch (err) {
 helper.error(err) 
} finally {
 setLoading(false) 
...
return
    ...
    setUser(_user); setLoading(true)
    try {
      const status = await TraccarService.getStatus(); setIntegrationEnabled(status.enabled)
      const loadedCars = await loadCars(); setCars(loadedCars)
      const defaultCar = loadedCars.find((car) => car.tracking?.deviceId) || loadedCars[0]
      if (defaultCar) {
setSelectedCarId(defaultCar._id)
}
      if (status.enabled) {
        const [devicesResult, fleetResult, geofencesResult, eventCenterResult] = await Promise.allSettled([
          TraccarService.getDevices(), TraccarService.getFleetOverview(), TraccarService.getAllGeofences(),
          TraccarService.getEventCenter({ from: new Date(from).toISOString(), to: new Date(to).toISOString(), limit: EVENT_CENTER_LIMIT }),
        ])
        if (devicesResult.status === 'fulfilled') {
setDevices(devicesResult.value)
}
        if (fleetResult.status === 'fulfilled') {
 setFleetOverview(fleetResult.value.items); setFleetHealth(fleetResult.value.health); setLastOperationalRefreshAt(new Date(fleetResult.value.health.lastRefreshAt)) 
}
        if (geofencesResult.status === 'fulfilled') {
setAllGeofences(geofencesResult.value)
}
...
setEventCenterItems(eventCenterResult.value)
 ...
 helper.error(err); setIntegrationEnabled(false) 
} finally {
 setLoading(false) 
}
  }
...
return
  ...
  // ─── SECTION ITEMS (tabs meta) ───────────────────────────────────────────
  const sectionItems = [
    { id: 'fleet' as const, icon: <DirectionsCarFilledIcon fontSize="small" />, title: strings.LIVE_FLEET, summary: `${onlineCarsCount} ${strings.ONLINE_DEVICES}` },
    { id: 'vehicle' as const, icon: <MyLocationIcon fontSize="small" />, title: strings.SELECTED_VEHICLE, summary: selectedCar?.name || strings.SELECT_CAR },
    { id: 'route' as const, icon: <RouteIcon fontSize="small" />, title: strings.ROUTE_HISTORY, summary: routeFrames.length > 0 ? `${routeFrames.length} ${strings.ROUTE_POINTS}` : strings.NO_DATA },
    { id: 'geofences' as const, icon: <RadarIcon fontSize="small" />, title: strings.GEOFENCES, summary: `${managedGeofences.length} ${strings.GEOFENCES}` },
    { id: 'events' as const, icon: <WarningAmberIcon fontSize="small" />, title: strings.EVENT_CENTER, summary: eventCenterItems.length > 0 ? `${eventCenterItems.length} ${strings.EVENT_CENTER}` : strings.NO_DATA },
  ]
        ...
        return (
          <>
            <Paper className="tracking-card tracking-card--embedded tracking-console-card">
              ...
              <div className="tracking-console-card__subheader">
                <FormControlLabel control={<Switch checked={liveRefreshEnabled} onChange={(event) => setLiveRefreshEnabled(event.target.checked)} />} label={strings.LIVE_REFRESH} />
                <span>{`${strings.LAST_UPDATE}: ${lastOperationalRefreshAt ? formatTimestamp(lastOperationalRefreshAt) : strings.NO_DATA}`}</span>
              </div>
              <div className="tracking-status-strip">
                {[
                  ...
                  <button type="button" key={option.value} className={`tracking-status-pill${fleetStatusFilter === option.value ? ' is-active' : ''}`} onClick={() => setFleetStatusFilter(option.value)}>
                    <strong>{option.count}</strong><span>{option.label}</span>
                  </button>
                ))}
              </div>
              <div className="tracking-fleet-filters">
                <TextField value={fleetSearch} onChange={(event) => setFleetSearch(event.target.value)} placeholder={strings.SEARCH_CARS} fullWidth size="small" className="tracking-search"
                  InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon fontSize="small" /></InputAdornment> }} />
                <FormControl size="small">
                  <InputLabel>{strings.LINK_FILTER}</InputLabel>
                  <Select value={fleetLinkFilter} label={strings.LINK_FILTER} onChange={(event) => setFleetLinkFilter(event.target.value as 'all' | 'linked' | 'unlinked')}>
                    <MenuItem value="all">{strings.ALL_LINKS}</MenuItem>
                    <MenuItem value="linked">{strings.LINKED_ONLY}</MenuItem>
                    <MenuItem value="unlinked">{strings.UNLINKED_ONLY}</MenuItem>
                  </Select>
                ...
                <FormControl size="small">
                  <InputLabel>{strings.SUPPLIER_FILTER}</InputLabel>
                  <Select value={fleetSupplierFilter} label={strings.SUPPLIER_FILTER} onChange={(event) => setFleetSupplierFilter(event.target.value)}>
                    <MenuItem value="all">{strings.ALL_SUPPLIERS}</MenuItem>
                    {supplierOptions.map((supplier) => <MenuItem key={supplier} value={supplier}>{supplier}</MenuItem>)}
                  </Select>
                </FormControl>
              </div>
              <div className="tracking-actions tracking-actions--compact">
                <Button variant="contained" className="btn-primary" onClick={handleRefreshFleet} disabled={!integrationEnabled}>{strings.REFRESH_FLEET}</Button>
                <Button variant="outlined" onClick={() => setMapMode('fleet')}>{strings.FLEET_MODE}</Button>
              </div>
            </Paper>
            <Paper className="tracking-card tracking-card--embedded">
              <div className="tracking-header">
              ...
              <div className="tracking-fleet-list tracking-fleet-list--dense">
                {filteredFleetCars.map((item) => {
                  const statusMeta = getFleetStatusMeta(item, selectedCarId)
                  const avatarBg = STATUS_COLORS[item.movementStatus] || '#475569'
                  return (
                    <button type="button" key={item.car._id} className={`tracking-fleet-item status-${item.movementStatus}${item.car._id === selectedCarId ? ' tracking-fleet-item--active' : ''}`}
                      onClick={() => {
 selectCar(item.car._id); setMapMode('single') 
}}>
                      <div className="tracking-fleet-avatar-shell">
                        <div className="tracking-fleet-avatar" style={{ background: `${avatarBg}22`, color: avatarBg }}>{item.car.name.slice(0, 2)}</div>
                      </div>
                      <div className="tracking-fleet-body">
                        <div className="tracking-fleet-row">
                          <Typography className="tracking-fleet-name">{item.car.name}</Typography>
        ...
        return (
          <>
            <Paper className="tracking-card tracking-card--embedded tracking-console-card">
              ...
              <div className="tracking-grid tracking-grid--compact">
                <div className="tracking-panel__summary-item"><span>{strings.DEVICE_NAME}</span><strong>{selectedFleetCar?.deviceName || strings.NO_DATA}</strong></div>
                <div className="tracking-panel__summary-item"><span>{strings.CURRENT_POSITION}</span><strong>{currentPoint ? `${formatCoordinate(currentPosition?.latitude)}, ${formatCoordinate(currentPosition?.longitude)}` : '-'}</strong></div>
                <div className="tracking-panel__summary-item"><span>{strings.TIME}</span><strong>{formatTimestamp(getPositionTimestamp(currentPosition))}</strong></div>
                <div className="tracking-panel__summary-item"><span>{strings.SPEED}</span><strong>{formatNumber(selectedFleetCar?.speedKmh, ' km/h')}</strong></div>
                <div className="tracking-panel__summary-item"><span>{strings.IGNITION}</span><strong>{selectedFleetCar?.snapshot?.ignition === undefined ? '-' : selectedFleetCar.snapshot.ignition ? strings.ON : strings.OFF}</strong></div>
                <div className="tracking-panel__summary-item"><span>{strings.BATTERY}</span><strong>{formatNumber(selectedFleetCar?.snapshot?.batteryLevel, '%')}</strong></div>
              </div>
              {selectedFleetCar?.snapshot?.address && <div className="tracking-inline-note">{selectedFleetCar.snapshot.address}</div>}
              <div className="tracking-actions tracking-actions--compact">
                <Button variant="contained" className="btn-primary" onClick={handleFetchPositions} disabled={!canLoadTracking}>{strings.FETCH}</Button>
                <Button variant="contained" className="btn-secondary" onClick={handleLoadSnapshot} disabled={!canLoadTracking}>{strings.LOAD_SNAPSHOT}</Button>
              </div>
            </Paper>
            <Paper className="tracking-card tracking-card--embedded">
              <div className="tracking-header">
                <div><Typography variant="h6">{strings.LINK_DEVICE}</Typography><Typography className="tracking-card-subtitle">{selectedCar?.name || strings.SELECT_CAR}</Typography></div>
                {selectedFleetCar?.snapshot?.deviceStatus && <Chip size="small" color={getStatusTone(selectedFleetCar.snapshot.deviceStatus)} label={selectedFleetCar.snapshot.deviceStatus} />}
              </div>
              {selectedCar ? (
                <>
                  {!selectedCar.tracking?.deviceId && <Alert severity="info" className="tracking-inline-alert">{strings.TRACKING_NOT_LINKED}</Alert>}
                  <div className="tracking-grid">
                    <FormControlLabel control={<Switch checked={trackingEnabled} onChange={(event) => setTrackingEnabled(event.target.checked)} />} label={strings.TRACKING_ENABLED} />
                    <FormControl>
                      <InputLabel>{strings.SELECT_DEVICE}</InputLabel>
                      <Select value={deviceId} label={strings.SELECT_DEVICE} onChange={(event) => {
 const nextDeviceId = event.target.value as string; setDeviceId(nextDeviceId); const nextDevice = devices.find((item) => `${item.id}` === nextDeviceId); if (nextDevice?.name) {
setDeviceName(nextDevice.name)
} 
}}>
                        {devices.map((device) => <MenuItem key={device.id} value={`${device.id}`}>{`${device.name || `Device ${device.id}`} ${device.status ? `(${device.status})` : ''}`}</MenuItem>)}
                      </Select>
                    </FormControl>
                    <TextField label={strings.DEVICE_ID} value={deviceId} onChange={(event) => setDeviceId(event.target.value)} />
                    <TextField label={strings.DEVICE_NAME} value={deviceName} onChange={(event) => setDeviceName(event.target.value)} />
                  ...
                  <div className="tracking-actions">
                    ...
                    <Button variant="contained" className="btn-secondary" onClick={handleUnlink} disabled={!selectedCar.tracking?.deviceId}>{strings.UNLINK_DEVICE}</Button>
                  </div>
                </>
              ) : <div className="tracking-empty">{strings.NO_DATA}</div>}
            </Paper>
            <Paper className="tracking-card tracking-card--embedded">
              <div className="tracking-header">
                <div><Typography variant="h6">{strings.COMMAND_CENTER}</Typography><Typography className="tracking-card-subtitle">{strings.COMMAND_CENTER_SUBTITLE}</Typography></div>
                <TerminalIcon fontSize="small" style={{ color: 'var(--t-text3)' }} />
              </div>
              {canLoadTracking ? (
                <>
                  <div className="tracking-grid">
                    <FormControl>
                      <InputLabel>{strings.COMMAND_TYPE}</InputLabel>
                      <Select value={selectedCommandType} label={strings.COMMAND_TYPE} onChange={(event) => setSelectedCommandType(event.target.value as string)}>
                        {commandTypes.map((commandType, index) => <MenuItem key={`${commandType.type || index}-${index}`} value={commandType.type || ''}>{commandType.type || strings.NO_DATA}</MenuItem>)}
                      </Select>
                    </FormControl>
                    <FormControlLabel control={<Switch checked={commandTextChannel} onChange={(event) => setCommandTextChannel(event.target.checked)} />} label={strings.TEXT_CHANNEL} />
                  </div>
                  <TextField label={strings.COMMAND_ATTRIBUTES} value={commandAttributes} onChange={(event) => setCommandAttributes(event.target.value)} multiline minRows={4} className="tracking-command-editor" fullWidth />
                  <div className="tracking-actions">
                    <Button variant="contained" className="btn-primary" onClick={handleSendCommand} disabled={!selectedCommandType} startIcon={<TerminalIcon />}>{strings.SEND_COMMAND}</Button>
                  </div>
                </>
              ) : <div className="tracking-empty">{strings.TRACKING_NOT_LINKED}</div>}
            </Paper>
      ...
      case 'route':
        return (
          <Paper className="tracking-card tracking-card--embedded">
            <div className="tracking-header">
              <Typography variant="h6">{strings.ROUTE_HISTORY}</Typography>
              <Button variant="contained" className="btn-primary" onClick={handleFetchRoute} disabled={!canLoadTracking}>{strings.FETCH}</Button>
            </div>
            <div className="tracking-grid">
              <TextField label={strings.FROM} type="datetime-local" value={from} onChange={(event) => setFrom(event.target.value)} />
              <TextField label={strings.TO} type="datetime-local" value={to} onChange={(event) => setTo(event.target.value)} />
            ...
            {routeFrames.length > 0 && snappedRoute.mode !== 'idle' && (
              <Alert severity={snappedRoute.mode === 'loading' ? 'info' : snappedRoute.mode === 'snapped' ? 'success' : 'warning'} className="tracking-inline-alert">
                {snappedRoute.mode === 'loading' ? strings.ROUTE_SNAP_LOADING : snappedRoute.mode === 'snapped' ? strings.ROUTE_SNAP_READY : strings.ROUTE_SNAP_FALLBACK}
              </Alert>
            )}
            {routeFrames.length > 0 && (
              <div className="tracking-route-visibility">
                <FormControlLabel control={<Switch checked={showHeatmap} onChange={(event) => setShowHeatmap(event.target.checked)} />} label={strings.HEATMAP} />
                <FormControlLabel control={<Switch checked={showStops} onChange={(event) => setShowStops(event.target.checked)} />} label={`${strings.STOP_DETECTION} (${detectedStops.length})`} />
              </div>
            )}
            {routeFrames.length > 0 ? (
              <>
                <div className="tracking-grid tracking-grid--compact">
                  <div className="tracking-panel__summary-item"><span>{strings.ROUTE_DISTANCE}</span><strong>{formatDistanceKm(routeDistanceMeters)}</strong></div>
                  <div className="tracking-panel__summary-item"><span>{strings.AVG_SPEED}</span><strong>{formatNumber(routeAverageSpeed, ' km/h')}</strong></div>
                  <div className="tracking-panel__summary-item"><span>{strings.MAX_SPEED}</span><strong>{formatNumber(routeMaxSpeed, ' km/h')}</strong></div>
                  <div className="tracking-panel__summary-item"><span>{strings.TRIPS}</span><strong>{vehicleReports.trips.length}</strong></div>
                  <div className="tracking-panel__summary-item"><span>{strings.STOPS}</span><strong>{vehicleReports.stops.length || detectedStops.length}</strong></div>
                  <div className="tracking-panel__summary-item"><span>{strings.STOP_DURATION}</span><strong>{routeStopDurationMs > 0 ? formatDuration(routeStopDurationMs) : '-'}</strong></div>
                </div>
                <div className="tracking-route-player">
                  <div className="tracking-route-player__stats">
                    <div className="tracking-route-player__stat"><span>{strings.PLAYBACK_POSITION}</span><strong>{`${boundedPlaybackIndex + 1}/${routeFrames.length}`}</strong></div>
                    <div className="tracking-route-player__stat"><span>{strings.SPEED}</span><strong>{`${formatNumber(playbackSpeedKmh, ' km/h')}`}</strong></div>
                    <div className="tracking-route-player__stat"><span>{strings.TIME}</span><strong>{formatTimestamp(getPositionTimestamp(playbackFrame?.position || null))}</strong></div>
                  </div>
                  <div className="tracking-route-player__controls">
                    <Button variant="contained" className="btn-primary" onClick={handlePlaybackToggle} disabled={routeFrames.length < 2} startIcon={playbackPlaying ? <PauseIcon /> : <PlayArrowIcon />}>
                      {playbackPlaying ? strings.ROUTE_PAUSE : strings.ROUTE_PLAYBACK}
                    </Button>
                    <Button variant="outlined" onClick={handlePlaybackReplay} disabled={routeFrames.length < 2} startIcon={<RestartAltIcon />}>{strings.PLAYBACK_RESTART}</Button>
                  </div>
                  <div className="tracking-route-player__toolbar">
                    <div className="tracking-route-player__speed">
                      <span>{strings.PLAYBACK_SPEED}</span>
                      <ToggleButtonGroup value={playbackSpeed} exclusive size="small" onChange={(_event, value: number | null) => value && setPlaybackSpeed(value)}>
                        {PLAYBACK_SPEED_OPTIONS.map((speed) => <ToggleButton key={speed} value={speed}>{`${speed}x`}</ToggleButton>)}
                      </ToggleButtonGroup>
                    </div>
                    <div className="tracking-route-player__progress"><span>{`${strings.PLAYBACK_PROGRESS}: ${playbackProgress}%`}</span></div>
                  </div>
                  <Slider min={0} max={Math.max(routeFrames.length - 1, 0)} value={boundedPlaybackIndex} onChange={handlePlaybackScrub} step={1}
                    marks={[{ value: 0, label: strings.ROUTE_START }, { value: Math.max(routeFrames.length - 1, 0), label: strings.ROUTE_END }]} disabled={routeFrames.length < 2} />
                </div>
                <div className="tracking-route-stops">
                  <div className="tracking-header">
                    <Typography variant="subtitle1">{strings.STOP_DETECTION}</Typography>
                  ...
                  <div className="tracking-route-stops">
                    ...
                    <div className="tracking-list">
                      {vehicleReports.trips.slice(0, 6).map((trip, index) => (
                        <div key={`${trip.startTime || index}-${trip.endTime || index}`} className="tracking-list-item">
                          <div>{`${formatTimestamp(trip.startTime)} → ${formatTimestamp(trip.endTime)}`}</div>
                          <div className="tracking-list-subtext">{`${formatDistanceKm(trip.distance)} • ${formatDuration(trip.duration || 0)}`}</div>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      case 'geofences':
        return (
          <>
            <Paper className="tracking-card tracking-card--embedded">
              <div className="tracking-header">
                <div><Typography variant="h6">{strings.GEOFENCE_MANAGER}</Typography><Typography className="tracking-card-subtitle">{editingGeofenceId ? strings.EDIT_GEOFENCE : strings.CREATE_GEOFENCE}</Typography></div>
                {editingGeofenceId && <Button variant="text" onClick={resetGeofenceForm}>{strings.CANCEL_EDIT}</Button>}
              </div>
              <div className="tracking-grid">
                <TextField label={strings.GEOFENCE_NAME} value={geofenceFormName} onChange={(event) => setGeofenceFormName(event.target.value)} />
                <FormControl>
                  <InputLabel>{strings.GEOFENCE_TYPE}</InputLabel>
                  <Select value={geofenceFormType} label={strings.GEOFENCE_TYPE} onChange={(event) => handleGeofenceTypeChange(event.target.value as GeofenceEditorType)}>
                    <MenuItem value="circle">{strings.GEOFENCE_TYPE_CIRCLE}</MenuItem>
                    <MenuItem value="polygon">{strings.GEOFENCE_TYPE_POLYGON}</MenuItem>
                    <MenuItem value="polyline">{strings.GEOFENCE_TYPE_POLYLINE}</MenuItem>
                  </Select>
                </FormControl>
                <TextField label={strings.DESCRIPTION} value={geofenceFormDescription} onChange={(event) => setGeofenceFormDescription(event.target.value)} />
              </div>
              <Alert severity={geofenceDrawingMode ? 'warning' : geofenceDraftReady ? 'success' : 'info'} className="tracking-inline-alert tracking-geofence-editor-alert">
                {geofenceDrawingMode ? strings.GEOFENCE_DRAWING_ACTIVE : geofenceDraftReady ? strings.GEOFENCE_READY : strings.DRAW_ON_MAP_HELP}
              </Alert>
              <div className="tracking-actions">
                <Button variant="contained" className="btn-primary" onClick={handleStartGeofenceDrawing} disabled={!integrationEnabled}>{strings.DRAW_ON_MAP}</Button>
                <Button variant="outlined" onClick={handleClearGeofenceDrawing} disabled={!geofenceDraft && !geofenceDrawingMode}>{strings.CLEAR_SHAPE}</Button>
              </div>
              <div className="tracking-geofence-summary">
                <div className="tracking-geofence-summary-item"><span>{strings.GEOFENCE_TYPE}</span><strong>{geofenceFormType === 'circle' ? strings.GEOFENCE_TYPE_CIRCLE : geofenceFormType === 'polygon' ? strings.GEOFENCE_TYPE_POLYGON : strings.GEOFENCE_TYPE_POLYLINE}</strong></div>
                <div className="tracking-geofence-summary-item"><span>{strings.GEOFENCE_POINTS}</span><strong>{geofenceDraft?.type === 'circle' ? '-' : `${geofenceDraftPointCount}`}</strong></div>
                {geofenceDraft?.type === 'circle' && <>
                  <div className="tracking-geofence-summary-item"><span>{strings.CENTER_LATITUDE}</span><strong>{formatCoordinate(geofenceDraft.center[0])}</strong></div>
                  <div className="tracking-geofence-summary-item"><span>{strings.CENTER_LONGITUDE}</span><strong>{formatCoordinate(geofenceDraft.center[1])}</strong></div>
                </>}
              </div>
              <div className="tracking-grid">
                {geofenceFormType === 'circle' && <TextField label={strings.RADIUS_METERS} value={geofenceFormRadius} onChange={(event) => handleGeofenceRadiusChange(event.target.value)} />}
                {geofenceFormType === 'polyline' && <TextField label={strings.POLYLINE_DISTANCE} value={geofenceFormPolylineDistance} onChange={(event) => setGeofenceFormPolylineDistance(event.target.value)} />}
              </div>
              <div className="tracking-actions">
                <Button variant="contained" className="btn-primary" onClick={handleSaveGeofence} disabled={!integrationEnabled || !geofenceDraftReady}>
                  {editingGeofenceId ? strings.UPDATE_GEOFENCE : strings.CREATE_GEOFENCE}
            ...
            <Paper className="tracking-card tracking-card--embedded">
              <div className="tracking-header">
                <div><Typography variant="h6">{strings.GEOFENCE_LIBRARY}</Typography><Typography className="tracking-card-subtitle">{selectedCar?.name || strings.SELECT_CAR}</Typography></div>
                <Button variant="contained" className="btn-primary" onClick={handleRefreshGeofenceLibrary} disabled={!integrationEnabled}>{strings.FETCH}</Button>
              </div>
              {managedGeofences.length > 0 ? (
                <div className="tracking-list">
                  {managedGeofences.map((geofence, index) => {
                    ...
                    return (
                      <div key={geofence.id || `${geofence.name}-${index}`} className="tracking-list-item tracking-geofence-library-item">
                        <div className="tracking-geofence-library-header">
                          ...
                          <Chip size="small" color={linked ? 'success' : 'default'} label={linked ? strings.LINKED_TO_SELECTED_CAR : strings.NOT_LINKED_TO_SELECTED_CAR} />
                        </div>
                        <div className="tracking-actions">
                          <Button variant="text" onClick={() => populateGeofenceForm(geofence)} disabled={!integrationEnabled || !editable}>{strings.EDIT_GEOFENCE}</Button>
                          <Button variant="text" color="error" onClick={() => typeof geofence.id === 'number' && handleDeleteGeofence(geofence.id)} disabled={!integrationEnabled || typeof geofence.id !== 'number'}>{commonStrings.DELETE}</Button>
                          {linked
                            ? <Button variant="contained" className="btn-secondary" onClick={() => typeof geofence.id === 'number' && handleUnlinkGeofence(geofence.id)} disabled={!canLoadTracking || typeof geofence.id !== 'number'}>{strings.UNLINK_FROM_CAR}</Button>
                            : <Button variant="contained" className="btn-primary" onClick={() => typeof geofence.id === 'number' && handleLinkGeofence(geofence.id)} disabled={!canLoadTracking || typeof geofence.id !== 'number'}>{strings.LINK_TO_CAR}</Button>}
                        </div>
                      </div>
                    )
                  })}
            ...
            <Paper className="tracking-card tracking-card--embedded">
              <div className="tracking-header">
                <Typography variant="h6">{strings.GEOFENCES}</Typography>
                <Button variant="contained" className="btn-primary" onClick={handleFetchGeofences} disabled={!canLoadTracking}>{strings.FETCH}</Button>
              </div>
              {geofences.length > 0 ? (
                <div className="tracking-list">
                  {geofences.map((geofence, index) => {
            ...
            {geofences.length > 0 && geofenceShapes.length !== geofences.length && (
              <Alert severity="info" className="tracking-info-alert tracking-overlay-alert">{strings.GEOFENCE_PARSE_NOTICE}</Alert>
            )}
          </>
        )
      ...
      case 'events':
        return (
          <Paper className="tracking-card tracking-card--embedded tracking-console-card">
            <div className="tracking-header">
              <div><Typography variant="h6">{strings.EVENT_CENTER}</Typography><Typography className="tracking-card-subtitle">{strings.EVENT_CENTER_SUBTITLE}</Typography></div>
              <Button variant="contained" className="btn-primary" onClick={handleFetchEvents} disabled={!integrationEnabled || (eventScope === 'vehicle' && !canLoadTracking)}>{strings.FETCH}</Button>
            </div>
            <div className="tracking-segmented-control">
              <button type="button" className={`tracking-segmented-control__item${eventScope === 'fleet' ? ' is-active' : ''}`} onClick={() => setEventScope('fleet')}>{strings.FLEET_EVENTS}</button>
              <button type="button" className={`tracking-segmented-control__item${eventScope === 'vehicle' ? ' is-active' : ''}`} onClick={() => setEventScope('vehicle')} disabled={!selectedCar}>{strings.VEHICLE_EVENTS}</button>
            </div>
            <div className="tracking-grid">
              <TextField label={strings.FROM} type="datetime-local" value={from} onChange={(event) => setFrom(event.target.value)} />
              <TextField label={strings.TO} type="datetime-local" value={to} onChange={(event) => setTo(event.target.value)} />
            ...
            <div className="tracking-event-filters">
              {EVENT_TYPE_OPTIONS.map((type) => (
                <button key={type} type="button" className={`tracking-filter-chip${eventTypeFilter === type ? ' is-active' : ''}`} onClick={() => setEventTypeFilter(type)}>{getEventTypeLabel(type)}</button>
              ))}
            </div>
            {eventCenterItems.length > 0 ? (
              <div className="tracking-list tracking-list--events">
  ...
  // ─── PANEL NAVIGATION ─────────────────────────────────────────────────────
  const renderPanelNavigation = () => (
    <div className="tracking-pane__tabs">
      <Tabs value={activePanelSection} onChange={(_event, value: TrackingPanelSection) => focusPanelSection(value)} variant="scrollable" scrollButtons="auto" allowScrollButtonsMobile className="tracking-pane__tabs-control">
        {sectionItems.map((section) => (
          <Tab key={section.id} value={section.id} icon={section.icon} iconPosition="start" label={section.title} />
        ))}
      </Tabs>
      <div className="tracking-pane__tabs-summary">{sectionItems.find((section) => section.id === activePanelSection)?.summary || strings.NO_DATA}</div>
    </div>
  )
  ...
  const renderOperationsHeader = () => (
    <>
      ...
      <div className="tracking-pane__toolbar">
        <ToggleButtonGroup value={mapMode} exclusive onChange={(_event, value: FleetMode | null) => value && setMapMode(value)} size="small" className="tracking-mode-toggle">
          <ToggleButton value="fleet">{strings.FLEET_MODE}</ToggleButton>
          <ToggleButton value="single">{strings.SINGLE_MODE}</ToggleButton>
        </ToggleButtonGroup>
        <FormControl className="tracking-car-select">
          <InputLabel>{strings.SELECT_CAR}</InputLabel>
          <Select value={selectedCarId} label={strings.SELECT_CAR} onChange={(event) => selectCar(event.target.value as string)}>
            {cars.map((car) => <MenuItem key={car._id} value={car._id}>{car.name}</MenuItem>)}
          </Select>
        </FormControl>
        <div className="tracking-pane__actions">
          <Button variant="contained" className="btn-primary" onClick={handleRefreshFleet} disabled={!integrationEnabled}>{strings.REFRESH_FLEET}</Button>
        ...
        <div className="tracking-pane__summary">
          <div className="tracking-panel__summary-item"><span>{strings.SELECTED_VEHICLE}</span><strong>{selectedCar?.name || strings.NO_DATA}</strong></div>
          <div className="tracking-panel__summary-item"><span>{strings.DEVICE_STATUS}</span><strong>{selectedStatusLabel}</strong></div>
          <div className="tracking-panel__summary-item"><span>{strings.CURRENT_POSITION}</span><strong>{currentPoint ? `${formatCoordinate(currentPosition?.latitude)}, ${formatCoordinate(currentPosition?.longitude)}` : '-'}</strong></div>
          <div className="tracking-panel__summary-item"><span>{strings.LAST_UPDATE}</span><strong>{lastOperationalRefreshAt ? formatTimestamp(lastOperationalRefreshAt) : strings.NO_DATA}</strong></div>
        </div>
      </div>
    </>
  )
  ...
  return (
    <Layout onLoad={onLoad} strict>
      {user && (
        <div className="tracking-page tracking-workspace">
          {!integrationEnabled && (
            <Alert severity="error" className="tracking-info-alert">{strings.INTEGRATION_DISABLED}</Alert>
          )}
          <div className={`tracking-shell${isMobileLayout ? ' tracking-shell--mobile' : ''}`}>
            {/* ── MAP ── */}
            <div className="tracking-shell__map">
              {!hasMapData && mapMode === 'fleet'
                ? (
                  <div className="tracking-map-empty">
                    <Typography variant="body1" style={{ color: 'var(--t-text2)' }}>{mapMode === 'fleet' ? strings.FLEET_EMPTY : strings.NO_MAP_DATA}</Typography>
                    <Typography variant="body2" style={{ color: 'var(--t-text3)' }}>{mapMode === 'fleet' ? strings.LIVE_FLEET_HINT : strings.MAP_EMPTY_HELP}</Typography>
                  </div>
                )
                : (
                  <GoogleTrackingMap
                    mapMode={mapMode} mapType={mapType} fleetCars={fleetCars} selectedFleetCar={selectedFleetCar}
                    currentPoint={currentPoint} currentPosition={currentPosition} routePathPoints={routePathPoints}
                    routeStartPoint={routeStartPoint} routeEndPoint={routeEndPoint} playbackPoint={playbackPoint}
                    playbackPosition={playbackFrame?.position || null} playbackHeading={playbackHeading}
                    heatmapPoints={heatmapPoints} showHeatmap={showHeatmap} stopMarkers={detectedStops} showStops={showStops}
                    geofenceShapes={visibleGeofenceShapes} draftGeofence={geofenceDraft} drawingMode={geofenceDrawingMode}
                    fitRequestToken={mapFitRequestToken}
                    onMarkerClick={(carId) => {
 selectCar(carId); setMapMode('single'); if (isMobileLayout) {
setActivePanelSection('vehicle')
} 
}}
                    onDraftGeofenceChange={handleDraftGeofenceChange} onDraftGeofenceDrawn={handleDraftGeofenceDrawn}
                  />
                )}

              {/* Map overlay top-right */}
              <div className="tracking-map-overlay tracking-map-overlay--top">
                <div className="tracking-map-badges--stack">
                  <div className="tracking-map-kpi-grid">
                    <div className="tracking-map-kpi-card">
                      <span>{strings.LIVE_FLEET}</span>
                      <strong>{fleetHealth?.totalCars ?? fleetCars.length}</strong>
                    ...
                    <div className="tracking-map-kpi-card">
                      <span>{strings.STATUS_MOVING}</span>
                      <strong style={{ color: 'var(--t-green)' }}>{fleetHealth?.movingCars ?? fleetStatusCounts.moving}</strong>
                      <em>{`${onlineCarsCount} ${strings.ONLINE_DEVICES}`}</em>
                    </div>
                    <div className="tracking-map-kpi-card">
                      <span>{strings.STATUS_IDLE}</span>
                      <strong style={{ color: 'var(--t-amber)' }}>{fleetHealth?.idleCars ?? fleetStatusCounts.idle}</strong>
                      <em>{`${fleetHealth?.stoppedCars ?? fleetStatusCounts.stopped} ${strings.STATUS_STOPPED}`}</em>
                    </div>
                    <div className="tracking-map-kpi-card">
                      <span>{strings.STATUS_STALE}</span>
                      <strong style={{ color: 'var(--t-text3)' }}>{fleetHealth?.staleCars ?? fleetStatusCounts.stale}</strong>
                      <em>{lastOperationalRefreshAt ? formatTimestamp(lastOperationalRefreshAt) : strings.NO_DATA}</em>
                    </div>
                  </div>
                  <div className="tracking-map-badges">
                    <Chip color={integrationEnabled ? 'success' : 'error'} label={integrationEnabled ? strings.SYSTEM_ONLINE : strings.INTEGRATION_DISABLED} size="small" />
                    {selectedCar && <Chip color={selectedStatusTone} label={`${selectedCar.name} • ${selectedStatusLabel}`} size="small" />}
                  </div>
                </div>
                {showMapLegend && (
                  <div className="tracking-map-legend-card">
                    <div className="tracking-map-legend-title">{mapMode === 'fleet' ? strings.FLEET_LEGEND : strings.SELECTED_VEHICLE}</div>
                    <div className="tracking-map-legend">
                      {mapMode === 'fleet' ? (
                        <>
                          <span><i className="tracking-legend-dot tracking-legend-dot--fleet" /> {strings.STATUS_MOVING}</span>
              ...
              {/* Map toolbar */}
              <div className="tracking-map-toolbar">
                <IconButton className="tracking-map-toolbar__button" onClick={handleRefreshFleet} disabled={!integrationEnabled}><RestartAltIcon fontSize="small" /></IconButton>
                <IconButton className="tracking-map-toolbar__button" onClick={() => setMapType((current) => (current === 'roadmap' ? 'hybrid' : 'roadmap'))}>
                  {mapType === 'roadmap' ? <SatelliteAltIcon fontSize="small" /> : <MapIcon fontSize="small" />}
                </IconButton>
                <IconButton className="tracking-map-toolbar__button" onClick={() => setShowMapLegend((current) => !current)}><LayersIcon fontSize="small" /></IconButton>
                <IconButton className="tracking-map-toolbar__button" onClick={() => (mapMode === 'fleet' ? focusFleetOnMap() : focusSelectedVehicleOnMap())}><CenterFocusStrongIcon fontSize="small" /></IconButton>
              </div>

              {/* Playback panel on map */}
              {mapMode === 'single' && routeFrames.length > 0 && (
                <div className="tracking-map-playback-panel">
                  <div className="tracking-map-playback-panel__row">
                    <strong>{strings.ROUTE_PLAYBACK}</strong>
                  ...
                  <div className="tracking-map-playback-panel__controls">
                    <Button size="small" variant="contained" className="btn-primary" onClick={handlePlaybackToggle} disabled={routeFrames.length < 2} startIcon={playbackPlaying ? <PauseIcon /> : <PlayArrowIcon />}>
                      {playbackPlaying ? strings.ROUTE_PAUSE : strings.ROUTE_PLAYBACK}
                    </Button>
                    <Button size="small" variant="outlined" onClick={handlePlaybackReplay} disabled={routeFrames.length < 2} startIcon={<RestartAltIcon />}>{strings.PLAYBACK_RESTART}</Button>
                  </div>
                  <Slider min={0} max={Math.max(routeFrames.length - 1, 0)} value={boundedPlaybackIndex} onChange={handlePlaybackScrub} step={1} disabled={routeFrames.length < 2} size="small" />
                </div>
              )}
            </div>
